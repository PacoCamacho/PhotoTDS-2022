					...
					for (Object object : resultadoBusqueda) {
						PanelResultadoBusqueda pr = new PanelResultadoBusqueda(object);
				                    ...
				                    if(pr.esPublicacion()) {
										...
										try {
											panelPerfil = new VentanaPerfil(u.getNombre(),u.getNombre(),true,false);
										} catch (DAOException e1) {
											// TODO Auto-generated catch block
											e1.printStackTrace();
										}
				...
				public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
					...
					if(value instanceof Item) {
						if(isSelected) {
							...
							try {
								panelFoto = new PanelFoto((Foto) resultadoBusqueda.get(index), ((Publicacion) resultadoBusqueda.get(index)).getUsuario(),false,true).getPanelFoto();
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
				...
				public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
					...
					if(value instanceof Item) {
						if(isSelected) {
							...
							try {
								panelPerfil = new VentanaPerfil(((Usuario) resultadoBusqueda.get(index)).getLogin(), usuario,true,true).getPanelPerfil();
